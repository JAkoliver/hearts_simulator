...
def main():
    ...
    # Load networks
    networks = []
    for seat_file in seats:
        net = HeartsNet()
        net.load_state_dict(torch.load(seat_file, weights_only=True))
        net.eval()
        networks.append(net)

    print("\nSeating Assignment:")
    for i, s in enumerate(seats):
        print(f" Seat {i}: {os.path.basename(s)}")
    ...
    for game in tqdm.tqdm(range(EVAL_GAMES), desc="Simulating Games", unit="game", ncols=80, bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]"):
        ...
        while not done:
            ...
            if len(current_trick_cards) == 0: # model wins the lead
                if 36 not in hand:
            ...
            # Metric: Forced Win Optimization
            forced = False
            if len(current_trick_cards) == 0:
                forced = True
            elif all((c // 13) == led_suit for c in legal_actions):
